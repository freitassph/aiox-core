...
const agentList = squadYaml.agents || [];
if (agentList.length === 0) {
  console.log(chalk.yellow('No agents found in squad.yaml'));
  process.exit(0);
}
...
const results = agentList.map(agent => {
  ...
  return analyzeAgent(path.join(squadDir, agentFile));
});
...
console.log(chalk.dim(`  Total agents:       ${agentList.length}`));
console.log(chalk.green(`  Passing:            ${passingCount}`));
console.log(chalk.red(`  Failing:            ${agentList.length - passingCount}`));
console.log(chalk.dim(`  Total lines:        ${totalLines}`));
console.log(chalk.dim(`  Average per agent:  ${Math.round(totalLines / agentList.length)}`));
console.log('='.repeat(50));
...
if (passingCount === agentList.length) {
  console.log(chalk.green.bold('\n✅ All agents meet the minimum standard!\n'));
  process.exit(0);
} else {
  console.log(chalk.red.bold(`\n❌ ${agentList.length - passingCount} agent(s) need expansion\n`));
  process.exit(1);
}
