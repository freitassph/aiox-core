...
function gate1SchemaCompliance() {
  ...
  if (squadYaml) {
    ...
    checks.push(check(!!squadYaml.agents, 'Has agents section'));
    checks.push(check(!!squadYaml.agents?.length, 'Has agents listed'));
    checks.push(check(!!squadYaml.tasks?.length, 'Has tasks listed'));
    checks.push(check(!!squadYaml.workflows?.length, 'Has workflows listed'));
    checks.push(check(!!squadYaml.checklists?.length, 'Has checklists listed'));
    checks.push(check(!!squadYaml.templates?.length, 'Has templates listed'));
...
function gate2AgentCompleteness() {
  ...
  agents.forEach(agent => {
    ...
    checks.push(check(passes, `${agentFile}: ${lines} lines ${passes ? '≥400 ✅' : '<400 ❌'}`));
  });
...
function gate3TaskCoverage() {
  ...
  tasks.forEach(task => {
    ...
    if (!content) {
      checks.push(check(false, `${taskFile}: file missing`));
      return;
    }
    const contentLower = content.toLowerCase();
    const hasAllSections = requiredSections.every(s => contentLower.includes(s));
    checks.push(check(hasAllSections, `${taskFile}: all required sections ${hasAllSections ? 'present ✅' : 'missing ❌'}`));
  });
...
function gate4WorkflowIntegrity() {
  ...
  workflows.forEach(wf => {
    ...
    const content = readFile(wfFile);
    if (!content) {
      checks.push(check(false, `${wfFile}: file missing`));
      return;
    }
    const contentLower = content.toLowerCase();
    checks.push(check(contentLower.includes('pre_conditions') || contentLower.includes('pre-conditions'), `${wfFile}: has pre_conditions`));
    checks.push(check(contentLower.includes('post_conditions') || contentLower.includes('post-conditions'), `${wfFile}: has post_conditions`));
    checks.push(check(contentLower.includes('steps'), `${wfFile}: has steps`));
    checks.push(check(contentLower.includes('error_handler') || contentLower.includes('on_failure'), `${wfFile}: has error handling`));
    checks.push(check(contentLower.includes('retry') || contentLower.includes('retry_on'), `${wfFile}: has retry logic`));
  });
...
function gate5Documentation() {
  ...
  if (squadYaml) {
    ...
    components.forEach(comp => {
      const items = squadYaml[comp] || [];
      items.forEach(item => {
        const relPath = typeof item === 'string' ? item : item.file;
        if (fileExists(relPath)) {
          const lines = countLines(relPath);
          checks.push(check(lines >= 10, `${relPath} has ${lines} lines (≥10 minimum)`));
        }
      });
    });
  }
...
function gate6CrossReference() {
  ...
  if (squadYaml) {
    ...
    const componentKeys = ['agents', 'tasks', 'workflows', 'checklists', 'templates'];
    componentKeys.forEach(key => {
      const items = squadYaml[key] || [];
      items.forEach(item => {
        const relPath = typeof item === 'string' ? item : item.file;
        checks.push(check(fileExists(relPath), `${relPath} declared and exists`));
      });
    });
...
function gate7ConstitutionAlignment() {
  ...
  const hasDeps = !!(squadYaml?.dependencies || squadYaml?.requires || squadYamlRaw?.includes('dependencies') || squadYamlRaw?.includes('requires'));
  checks.push(check(
    hasDeps,
  ...
  checks.push(check(
    readmeLower.includes('structure') || readmeLower.includes('directory') || readmeLower.includes('diretório') || readmeLower.includes('estrutura'),
    'Absolute Imports: Documents directory structure'
  ));

  checks.forEach(c => {
    if (!c.passed) console.log(chalk.red(`    ✗ ${c.message}`));
  });
