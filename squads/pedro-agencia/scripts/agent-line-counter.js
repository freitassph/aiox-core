...
function analyzeAgent(filePath) {
  ...
  // Normalize content for section matching
  const contentNormalized = content.toLowerCase().replace(/[_\s]+/g, ' ');
  ...
  // Check required sections (case-insensitive, normalize underscores and spaces)
  const foundSections = requiredSections.filter(section => {
    const sectionNormalized = section.toLowerCase().replace(/[_\s]+/g, ' ');
    return contentNormalized.includes(sectionNormalized);
  });
  ...
  return {
    ...
    passes: totalLines >= minLines && missingSections.length === 0 && fillerRatio < 15
  };
}
